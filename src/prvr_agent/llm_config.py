...
class LLMConfig:
    ...
    def validate(self) -> None:
        ...
        if self.timeout <= 0:
            raise ValueError("PRVR_LLM_TIMEOUT must be positive")
        ...
        if self.temperature < 0:
            raise ValueError("PRVR_LLM_TEMPERATURE must be non-negative")
...
def create_openai_compatible_client(config: Optional[LLMConfig] = None):
...
def resolve_served_model(client, requested_model: Optional[str] = None) -> str:
